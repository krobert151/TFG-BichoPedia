package com.robertorebolledonaharro.bichoapi.common.error.exeptions.user.util;
...
public class GenericSpecificationsBuilder<U> {
    ...
    public Specification<U> build(Deque<?> postFixedExprStack, Function<SpecSearchCriteria, Specification<U>> converter) {
        Deque<?> stackCopy = new LinkedList<>(postFixedExprStack);
        Deque<Specification<U>> specStack = new LinkedList<>();

        Collections.reverse((List<?>) stackCopy);

        while (!stackCopy.isEmpty()) {
            Object mayBeOperand = stackCopy.pop();
